    ...
    <>
      <div className="flex gap-2 pt-1">
        <button
          ...
          className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-md text-[10px] font-medium text-muted-foreground bg-white/[0.04] hover:bg-white/[0.08] transition-colors"
        >
        ...
        <DialogContent className="bg-background border-border max-w-[420px] max-h-[70vh]">
          ...
          <div className="overflow-y-auto max-h-[50vh] space-y-1.5 pt-1" style={{ scrollbarWidth: 'none' }}>
            {loading ? (
              ...
              <p className="text-[11px] text-muted-foreground text-center py-8">Nenhuma configuração salva</p>
            ) : (
              configs.map(config => (
                <div
                  key={config.id}
                ...
                >
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] text-muted-foreground font-medium truncate">{config.name}</p>
