...
const UpscalerRunpod: React.FC = () => {
  ...
  const processImage = async () => {
    ...
    try {
      ...
      // Step 1: Start Runpod job (sends image + workflow)
      console.log('[Runpod] Starting job...');
      const runResponse = await supabase.functions.invoke('runpod-upscaler/run', {
        body: {
      ...
      const { jobId } = runResponse.data;
      
      // Validate jobId exists
      if (!jobId) {
        setLastError({
          message: 'O servidor não retornou um ID de job',
          code: 'NO_JOB_ID',
          solution: 'Verifique a configuração do Runpod endpoint'
        });
        throw new Error('Servidor não retornou jobId');
      }
      
      console.log('[Runpod] Job started, jobId:', jobId);
      ...
      pollingRef.current = setInterval(async () => {
        ...
        try {
          const statusResponse = await supabase.functions.invoke('runpod-upscaler/status', {
            body: { jobId },
          });

          if (statusResponse.error) {
            console.error('[Runpod] Status check error:', statusResponse.error);
            return;
          }
          ...
          if (!jobStatus) {
            console.warn('[Runpod] Status undefined, continuing polling...');
            return;
          }
          
          console.log('[Runpod] Job status:', jobStatus);

          // Update progress based on status
          if (jobStatus === 'IN_QUEUE' || jobStatus === 'IN_PROGRESS') {
            ...
            console.log('[Runpod] Output:', output);

            if (output && output.images && output.images.length > 0) {
              // Get the last output (usually the final processed image)
              setOutputImage(output.images[output.images.length - 1]);
              setStatus('completed');
              setProgress(100);
      ...
      }, 5000); // 5 seconds polling interval (Runpod is faster)

    } catch (error: any) {
      console.error('[Runpod] Process error:', error);
      setStatus('error');
      ...
      <div className="sticky top-0 z-50 bg-[#0D0221]/80 backdrop-blur-lg border-b border-purple-500/20">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center gap-4">
          ...
          <h1 className="text-xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
            Upscaler Arcano (Runpod)
          </h1>
          <span className="ml-auto text-xs bg-yellow-500/20 text-yellow-300 px-2 py-1 rounded">
            🧪 TESTE
          </span>
        </div>
      </div>
                ...
                <div className="absolute inset-0 bg-black/70 flex flex-col items-center justify-center gap-4">
                  ...
                  <div className="text-center">
                    <p className="text-lg font-medium">
                      {status === 'uploading' ? 'Enviando imagem...' : 'Processando via Runpod...'}
                    </p>
