...
const PartnerDashboardMusicos = () => {
  const navigate = useNavigate();
  const [partner, setPartner] = useState<{ id: string; name: string } | null>(null);
  const [artes, setArtes] = useState<PartnerArteMusicos[]>([]);
  const [categories, setCategories] = useState<Category[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [activeFilter, setActiveFilter] = useState<FilterType>("all");
  const [showEditModal, setShowEditModal] = useState(false);
  const [editingArte, setEditingArte] = useState<PartnerArteMusicos | null>(null);
  ...
  const checkPartnerAndFetchData = async () => {
    try {
      ...
      // Get partner info from partners table
      ...
      // Check if partner has access to artes_musicos platform
  ...
  const getArteStatus = (arte: PartnerArteMusicos) => {
  ...
  const openEditModal = (arte: PartnerArteMusicos) => {
    setEditingArte(arte);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#1a1a2e] via-[#16213e] to-[#0f0f1a] p-4">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Upload Button */}
        <Button
          className="w-full mb-6 bg-gradient-to-r from-pink-500 to-rose-600 hover:from-pink-600 hover:to-rose-700 text-foreground"
          onClick={() => navigate("/parceiro-upload-musicos")}
        >
          ...
          {[
            ...
            <Button
              ...
              className={activeFilter === filter.key 
                ? "bg-gradient-to-r from-pink-500 to-rose-600 text-foreground" 
      ...
      <Dialog open={showEditModal} onOpenChange={setShowEditModal}>
        <DialogContent className="bg-card border-pink-500/30 text-foreground">
          <DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              ...
              <Button
                className="flex-1 bg-gradient-to-r from-pink-500 to-rose-600 hover:from-pink-600 hover:to-rose-700 text-foreground"
...
export default PartnerDashboardMusicos;