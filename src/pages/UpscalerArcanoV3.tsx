  ...
  return (
    <div className="v3-real-card v3-reveal">
      <div
        ...
        onTouchStart={(e) => { dragging.current = true; update(e.touches[0].clientX); }}
      >
        <img src={item.before} alt={`${item.name} antes`} style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} loading="lazy" />
        <div style={{ position: "absolute", inset: 0, clipPath: `inset(0 ${100 - pct}% 0 0)` }}>
          <img src={item.after} alt={`${item.name} depois`} style={{ width: "100%", height: "100%", objectFit: "cover" }} loading="lazy" />
        </div>
      ...
      <div className="v3-real-card-info">
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <img src={item.avatar} alt={item.name} className="v3-real-card-avatar" />
          <div>
  ...
  return (
    <>
      <div className="v3-page">
        ...
        <V3SocialPopup people={SOCIAL_PEOPLE} times={SOCIAL_TIMES} purchaseText="acabou de comprar" />

        {/* STICKY CTA - isolated component */}
        <V3StickyBar
          scrollToPrice={scrollToPrice}
          label="Upscaler Arcano V3"
          desktopSuffix="R$ 24,90 pra começar"
          mobileButtonText="Ver Planos →"
          desktopButtonText="Garantir Acesso →"
        />
