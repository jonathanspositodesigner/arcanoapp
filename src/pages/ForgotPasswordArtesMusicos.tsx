...
const ForgotPasswordArtesMusicos = () => {
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#1a1a2e] via-[#2d1b4e] to-[#0f0f1a] flex items-center justify-center p-4">
        <Card className="w-full max-w-md bg-[#1a1a2e]/80 border-white/10">
          <CardContent className="p-8 text-center">
            ...
            <Button variant="outline" className="border-white/10 text-gray-400 hover:bg-white/50/20" onClick={() => navigate("/login-artes-musicos")}>{t('emailSent.backToLogin')}</Button>
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#1a1a2e] via-[#2d1b4e] to-[#0f0f1a] flex items-center justify-center p-4">
      <Card className="w-full max-w-md bg-[#1a1a2e]/80 border-white/10 relative">
        <CardHeader className="text-center">
          ...
          <div className="flex items-center justify-center gap-2 mb-2 mt-4"><Music className="h-5 w-5 text-gray-400" /></div>
        ...
        <CardContent>
          <form onSubmit={handleSendResetEmail} className="space-y-4">
            ...
            <Button type="submit" className="w-full bg-slate-600 hover:bg-slate-700 text-white" disabled={isLoading}>{isLoading ? t('sending') : t('forgotPasswordCard.sendRecoveryEmail')}</Button>
          </form>
        </CardContent>
      </Card>
    </div>
