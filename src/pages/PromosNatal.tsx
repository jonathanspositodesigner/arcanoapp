...
const PromosNatal = () => {
  ...
  if (loading || promoLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-[#0f1419] via-[#1a1f2e] to-[#0d1117] flex items-center justify-center">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0f1419] via-[#1a1f2e] to-[#0d1117] p-4 relative overflow-hidden">
          ...
          {(countdown.days > 0 || countdown.hours > 0 || countdown.minutes > 0) && (
            <div className="flex justify-center gap-3 mb-6">
              {[
                ...
                <div key={i} className="bg-black/50 border border-red-500/30 rounded-lg px-4 py-2 min-w-[70px]">
                  <div className="text-2xl font-bold text-white">{String(item.value).padStart(2, '0')}</div>
                  <div className="text-[10px] text-red-400">{item.label}</div>
                </div>
              ))}
            </div>
          )}
        ...
        {!selectedPack ? (
          ...
          <>
            <div className="flex justify-center mb-6">
              <Button
                variant="outline"
                className="bg-red-500/20 border-red-500 text-white hover:bg-red-500/30"
            ...
            {selectedPack?.cover_url && (
              <div className="flex justify-center mb-8">
                <img
                  ...
                  className="w-32 h-32 object-cover rounded-lg border-2 border-red-500/50"
                />
              </div>
            )}
            ...
            <div className={`grid gap-6 ${accessOptions.length === 1 ? 'max-w-md mx-auto' : accessOptions.length === 2 ? 'md:grid-cols-2 max-w-2xl mx-auto' : 'md:grid-cols-3'}`}>
              {accessOptions.map((option) => {
                ...
                return (
                  <Card
                    key={option.type}
                    className={`relative bg-black/40 border-red-500/30 ${
                      option.highlighted ? "ring-2 ring-red-500 scale-105" : ""
                    }`}
                  >
                    {option.highlighted && (
