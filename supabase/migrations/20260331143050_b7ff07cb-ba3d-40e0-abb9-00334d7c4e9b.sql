...
-- Update cleanup_all_stale_ai_jobs to use 15 minutes for MovieLed Maker (video tool needs more time)
...
BEGIN
  ...
  FOR job IN SELECT id, user_id, user_credit_cost, credits_charged, credits_refunded FROM video_upscaler_jobs WHERE status IN ('running','queued','starting','pending') AND created_at < NOW() - video_stale_threshold LOOP
  ...
  FOR job IN SELECT id, user_id, user_credit_cost, credits_charged, credits_refunded FROM video_generator_jobs WHERE status IN ('running','queued','starting','pending') AND created_at < NOW() - video_stale_threshold LOOP
  ...
  FOR job IN SELECT id, user_id, user_credit_cost, credits_charged, credits_refunded FROM movieled_maker_jobs WHERE status IN ('running','queued','starting','pending') AND created_at < NOW() - video_stale_threshold LOOP
