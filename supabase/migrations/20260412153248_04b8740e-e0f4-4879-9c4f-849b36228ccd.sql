...
DROP FUNCTION IF EXISTS public.get_unified_dashboard_orders(text, text);
...
BEGIN
  ...
  WITH stripe_orders_ranked AS (
    ...
    WHERE so.created_at >= _start
      AND so.created_at < _end
  ...
  combined AS (
    ...
    WHERE ao.created_at >= _start
      AND ao.created_at < _end
    ...
    WHERE wl.received_at >= _start
      AND wl.received_at < _end
