...
BEGIN
  ...
  -- MONTHLY LIMIT: 1 earning per (user × prompt) per calendar month (across all AI tools)
  IF EXISTS (
    ...
    RETURN jsonb_build_object(
      'success', false,
      'error', 'monthly_user_prompt_limit_reached',
      'message', 'Already credited for this user+prompt this month'
    );
  END IF;
