...
-- Update get_ai_tools_usage to split Gerar Imagem by engine
...
-- Update get_ai_tools_usage_summary to split Gerar Imagem by engine
...
RETURNS TABLE(
  ...
  total_rh_cost NUMERIC, total_user_credits NUMERIC, total_profit NUMERIC,
  jobs_with_queue BIGINT, avg_queue_wait_seconds NUMERIC, avg_processing_seconds NUMERIC
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  WITH all_jobs AS (
    ...
    UNION ALL
    SELECT igj.status, COALESCE(igj.rh_cost, 0) as rh_cost, COALESCE(igj.user_credit_cost, 0) as user_credit_cost,
      false as waited_in_queue, 0 as queue_wait_seconds,
  ...
  SELECT
    ...
    COALESCE(SUM(aj.rh_cost), 0)::NUMERIC as total_rh_cost,
    COALESCE(SUM(aj.user_credit_cost), 0)::NUMERIC as total_user_credits,
    COALESCE(SUM(aj.user_credit_cost - aj.rh_cost), 0)::NUMERIC as total_profit,
    COUNT(*) FILTER (WHERE aj.waited_in_queue = true)::BIGINT as jobs_with_queue,
    COALESCE(AVG(aj.queue_wait_seconds) FILTER (WHERE aj.waited_in_queue = true), 0)::NUMERIC as avg_queue_wait_seconds,
    COALESCE(AVG(aj.processing_seconds) FILTER (WHERE aj.processing_seconds > 0), 0)::NUMERIC as avg_processing_seconds
  FROM all_jobs aj;
END;
$$;
