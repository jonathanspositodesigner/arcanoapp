CREATE INDEX IF NOT EXISTS idx_upscaler_credit_transactions_user_created_at
ON public.upscaler_credit_transactions (user_id, created_at DESC);
...
BEGIN
  ...
  RETURN QUERY EXECUTE format($q$
    WITH tx_users AS (
      SELECT t.user_id AS uid, MAX(t.created_at) AS last_transaction_at
      FROM public.upscaler_credit_transactions t
      GROUP BY t.user_id
    ),
    all_uids AS (
      SELECT uc.user_id AS uid FROM public.upscaler_credits uc
      UNION
      SELECT tx.uid FROM tx_users tx
    ),
    enriched AS (
      SELECT
        au.uid,
        ...
        p.name,
        p.phone,
        COALESCE(uc.monthly_balance, 0)::integer AS monthly_balance,
        COALESCE(uc.lifetime_balance, 0)::integer AS lifetime_balance,
        (COALESCE(uc.monthly_balance, 0) + COALESCE(uc.lifetime_balance, 0))::integer AS total_balance,
        tx.last_transaction_at
      FROM all_uids au
      LEFT JOIN public.upscaler_credits uc ON uc.user_id = au.uid
      LEFT JOIN tx_users tx ON tx.uid = au.uid
      LEFT JOIN public.profiles p ON p.id = au.uid
    ),
    filtered AS (
