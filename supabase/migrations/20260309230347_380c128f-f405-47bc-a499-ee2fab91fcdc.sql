  ...
  SELECT * FROM (
    ...
    FROM (
      ...
      WHERE wl2.received_at >= _start AND wl2.received_at < _end
        ...
        AND wl2.status IN ('paid', 'approved', 'PURCHASE_APPROVED', 'PURCHASE_COMPLETE', 'refunded', 'chargeback', 'PURCHASE_REFUNDED', 'PURCHASE_CHARGEBACK', 'waiting_payment', 'pending_payment', 'PURCHASE_DELAYED')
