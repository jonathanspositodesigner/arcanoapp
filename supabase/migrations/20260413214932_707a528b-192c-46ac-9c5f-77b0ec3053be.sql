...
-- VUL-001: Restrict refund_upscaler_credits to service_role or admin only
...
DECLARE
  ...
  v_credit_type TEXT;
BEGIN
  -- SECURITY: Only service_role (auth.uid() IS NULL) or admin can call this
  IF auth.uid() IS NOT NULL AND NOT public.has_role(auth.uid(), 'admin') THEN
    RETURN QUERY SELECT FALSE, 0, 'Access denied: only service_role or admin can issue refunds'::TEXT;
    RETURN;
  END IF;
