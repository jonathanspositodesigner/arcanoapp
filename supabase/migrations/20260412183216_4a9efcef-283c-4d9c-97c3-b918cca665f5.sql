...
BEGIN
  ...
  -- Add 500 lifetime credits to referred user
  INSERT INTO upscaler_credits (user_id, balance, monthly_balance, lifetime_balance)
  VALUES (p_referred_user_id, 500, 0, 500)
  ON CONFLICT (user_id) DO UPDATE SET
    lifetime_balance = upscaler_credits.lifetime_balance + 500,
    balance = upscaler_credits.monthly_balance + upscaler_credits.lifetime_balance + 500,
    updated_at = now();
  ...
  VALUES (p_referred_user_id, 500, v_referred_balance, 'bonus', 'Bônus de indicação: 500 créditos vitalícios', 'lifetime');

  -- Add 500 lifetime credits to referrer
  INSERT INTO upscaler_credits (user_id, balance, monthly_balance, lifetime_balance)
  VALUES (v_referrer_id, 500, 0, 500)
  ON CONFLICT (user_id) DO UPDATE SET
    lifetime_balance = upscaler_credits.lifetime_balance + 500,
    balance = upscaler_credits.monthly_balance + upscaler_credits.lifetime_balance + 500,
    updated_at = now();
  ...
  INSERT INTO upscaler_credit_transactions (user_id, amount, balance_after, transaction_type, description, credit_type)
  VALUES (v_referrer_id, 500, v_referrer_balance, 'bonus', 'Bônus de indicação: 500 créditos vitalícios (indicou um amigo)', 'lifetime');
