...
DROP FUNCTION IF EXISTS public.get_ai_tools_usage(TIMESTAMPTZ, TIMESTAMPTZ, INTEGER, INTEGER);
DROP FUNCTION IF EXISTS public.get_ai_tools_usage_count(TIMESTAMPTZ, TIMESTAMPTZ);
DROP FUNCTION IF EXISTS public.get_ai_tools_usage_summary(TIMESTAMPTZ, TIMESTAMPTZ);

-- Recreate get_ai_tools_usage with p_tool_filter
...
BEGIN
  ...
  WITH all_jobs AS (
    ...
    SELECT igj.id,
      ...
      COALESCE(igj.rh_cost, 0)::NUMERIC as rh_cost, COALESCE(igj.user_credit_cost, 0)::NUMERIC as user_credit_cost,
    ...
    UNION ALL
    SELECT sj.id, 'Seedance 2.0'::TEXT, sj.user_id, sj.status, sj.error_message, sj.failed_at_step,
      COALESCE(sj.rh_cost, 0)::NUMERIC, COALESCE(sj.user_credit_cost, 0)::NUMERIC,
      (COALESCE(sj.user_credit_cost, 0) - COALESCE(sj.rh_cost, 0))::NUMERIC,
      COALESCE(sj.waited_in_queue, false), COALESCE(sj.queue_wait_seconds, 0),
      CASE WHEN sj.started_at IS NOT NULL AND sj.completed_at IS NOT NULL THEN EXTRACT(EPOCH FROM (sj.completed_at - sj.started_at))::INTEGER ELSE 0 END,
...
-- Recreate get_ai_tools_usage_count with p_tool_filter
DROP FUNCTION IF EXISTS public.get_ai_tools_usage_count(TIMESTAMPTZ, TIMESTAMPTZ);
...
BEGIN
  SELECT COUNT(*) INTO total_count FROM (
    SELECT id, 'Upscaler Arcano'::TEXT as tn FROM upscaler_jobs WHERE user_id IS NOT NULL AND (p_start_date IS NULL OR created_at >= p_start_date) AND (p_end_date IS NULL OR created_at <= p_end_date)
    UNION ALL
    SELECT id, 'Pose Changer'::TEXT FROM pose_changer_jobs WHERE user_id IS NOT NULL AND (p_start_date IS NULL OR created_at >= p_start_date) AND (p_end_date IS NULL OR created_at <= p_end_date)
    UNION ALL
    SELECT id, 'Veste AI'::TEXT FROM veste_ai_jobs WHERE user_id IS NOT NULL AND (p_start_date IS NULL OR created_at >= p_start_date) AND (p_end_date IS NULL OR created_at <= p_end_date)
    UNION ALL
    SELECT id, 'Video Upscaler'::TEXT FROM video_upscaler_jobs WHERE user_id IS NOT NULL AND (p_start_date IS NULL OR created_at >= p_start_date) AND (p_end_date IS NULL OR created_at <= p_end_date)
    UNION ALL
    SELECT id, 'Arcano Cloner'::TEXT FROM arcano_cloner_jobs WHERE user_id IS NOT NULL AND (p_start_date IS NULL OR created_at >= p_start_date) AND (p_end_date IS NULL OR created_at <= p_end_date)
    UNION ALL
    SELECT id, 'Gerador Avatar'::TEXT FROM character_generator_jobs WHERE user_id IS NOT NULL AND (p_start_date IS NULL OR created_at >= p_start_date) AND (p_end_date IS NULL OR created_at <= p_end_date)
    UNION ALL
    SELECT id, 'Flyer Maker'::TEXT FROM flyer_maker_jobs WHERE user_id IS NOT NULL AND (p_start_date IS NULL OR created_at >= p_start_date) AND (p_end_date IS NULL OR created_at <= p_end_date)
    UNION ALL
    SELECT id, 'Remover Fundo'::TEXT FROM bg_remover_jobs WHERE user_id IS NOT NULL AND (p_start_date IS NULL OR created_at >= p_start_date) AND (p_end_date IS NULL OR created_at <= p_end_date)
    UNION ALL
    SELECT id, CASE WHEN engine = 'nano_banana' THEN 'Gerar Imagem - Nano Banana' ELSE 'Gerar Imagem - Flux 2' END::TEXT FROM image_generator_jobs WHERE user_id IS NOT NULL AND (p_start_date IS NULL OR created_at >= p_start_date) AND (p_end_date IS NULL OR created_at <= p_end_date)
    UNION ALL
    SELECT id, 'Gerar Vídeo'::TEXT FROM video_generator_jobs WHERE user_id IS NOT NULL AND (p_start_date IS NULL OR created_at >= p_start_date) AND (p_end_date IS NULL OR created_at <= p_end_date)
    UNION ALL
    SELECT id, 'MovieLed Maker'::TEXT FROM movieled_maker_jobs WHERE user_id IS NOT NULL AND (p_start_date IS NULL OR created_at >= p_start_date) AND (p_end_date IS NULL OR created_at <= p_end_date)
    UNION ALL
    SELECT id, 'Seedance 2.0'::TEXT FROM seedance_jobs WHERE user_id IS NOT NULL AND (p_start_date IS NULL OR created_at >= p_start_date) AND (p_end_date IS NULL OR created_at <= p_end_date)
  ) AS all_jobs
  WHERE (p_tool_filter IS NULL OR p_tool_filter = '' OR p_tool_filter = 'all' OR tn = p_tool_filter);
  RETURN total_count;
END;
$$;

-- Recreate get_ai_tools_usage_summary with p_tool_filter
...
BEGIN
  ...
  WITH all_jobs AS (
    ...
    UNION ALL
    SELECT 'Seedance 2.0'::TEXT, sj.status, COALESCE(sj.rh_cost, 0), COALESCE(sj.user_credit_cost, 0), COALESCE(sj.waited_in_queue, false), COALESCE(sj.queue_wait_seconds, 0),
      CASE WHEN sj.started_at IS NOT NULL AND sj.completed_at IS NOT NULL THEN EXTRACT(EPOCH FROM (sj.completed_at - sj.started_at))::INTEGER ELSE 0 END
