-- Update consume_upscaler_credits to handle new Veo 3.1 model names
...
BEGIN
  -- Check if user is unlimited subscriber
  _is_unlimited := is_unlimited_subscriber(_user_id);

  -- If unlimited and NOT a Veo 3.1 tool, skip consumption
  ...
  -- Priority: monthly first, then lifetime
  ...
  -- Log transaction
  INSERT INTO upscaler_credit_transactions (user_id, amount, type, description)
