...
async function handleRun(req: Request) {
  ...
  const MAX_POLLS = 60; // GPT Image can take longer
  ...
  for (let i = 0; i < MAX_POLLS; i++) {
    ...
    try {
      ...
      console.log(`[GPTImage] Poll ${i + 1}/${MAX_POLLS}: status=${rhStatus}`);
      ...
      if (rhStatus === 'SUCCESS' && queryData.results?.length > 0) {
        ...
        await logStep(jobId, 'completed', { outputUrl, rhCost });

        return new Response(JSON.stringify({ success: true, outputUrl }), {
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }

      if (rhStatus === 'FAILED') {
        const errMsg = queryData.errorMessage || queryData.failedReason?.error || 'Generation failed on RunningHub';
        ...
        await logStep(jobId, 'generation_failed', { error: normalizedErr.message });

        return new Response(JSON.stringify({ error: normalizedErr.message, code: 'GENERATION_FAILED', refunded: creditCost > 0 }), {
          status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }

    } catch (pollErr) {
      console.warn(`[GPTImage] Poll ${i + 1} error:`, pollErr);
    }
  }

  // Timeout
  console.error(`[GPTImage] Job ${jobId} timed out after ${MAX_POLLS} polls`);
  if (creditCost > 0) {
    await supabase.rpc('refund_upscaler_credits', {
      _user_id: verifiedUserId,
      _amount: creditCost,
      _description: 'GPT_IMAGE_TIMEOUT_REFUND',
    });
  }
  await supabase.from(TABLE_NAME).update({
    status: 'failed',
  ...
  await logStep(jobId, 'timeout');

  return new Response(JSON.stringify({
    error: 'Tempo esgotado na geração. Seus créditos foram estornados.',
    code: 'TIMEOUT',
    refunded: creditCost > 0,
  }), {
