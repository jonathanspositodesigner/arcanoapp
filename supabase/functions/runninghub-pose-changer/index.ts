...
async function logStep(
  ...
  try {
    ...
    console.log(`[PoseChanger] Job ${jobId}: ${step}`, details || '');
  } catch (e) {
...
async function logStepFailure(
  ...
  try {
    ...
    await supabase.from('pose_changer_jobs').update(updateData).eq('id', jobId);
    
    console.log(`[PoseChanger] Job ${jobId}: FAILED at ${failedAtStep}:`, errorMessage);
  } catch (e) {
...
async function handleRun(req: Request) {
  ...
  const { jobId, personImageUrl, referenceImageUrl, creditCost } = body;
  ...
  console.log(`[PoseChanger] JWT verified - userId: ${userId}`);
