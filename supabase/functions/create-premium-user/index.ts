...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (existingPremium) {
      ...
      if (updateError) {
        console.error('Error updating premium record:', updateError);
        return new Response(
          JSON.stringify({ error: `Failed to update premium record: ${updateError.message}` }),
          { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }

      console.log('Updated existing premium record');
      return new Response(
        JSON.stringify({ 
          success: true, 
      ...
      if (insertError) {
        console.error('Error inserting premium record:', insertError);
        return new Response(
          JSON.stringify({ error: `Failed to create premium record: ${insertError.message}` }),
          { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }

      console.log('Created new premium record');
      return new Response(
        JSON.stringify({ 
          success: true, 
