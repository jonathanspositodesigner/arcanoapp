...
const DEFAULT_PIXEL_ID = '1162356848586894'
...
serve(async (req) => {
  ...
  try {
    ...
    const effectivePixelId = pixel_id || DEFAULT_PIXEL_ID
    console.log(`📊 Meta CAPI: ${event_name} | pixel: ${effectivePixelId} | email: ${email ? email.substring(0, 5) + '***' : 'N/A'} | value: ${value || 'N/A'} | event_id: ${event_id || 'N/A'} | fbp: ${fbp ? '✅' : '❌'} | fbc: ${fbc ? '✅' : '❌'}`)

    const response = await fetch(
      `https://graph.facebook.com/${API_VERSION}/${effectivePixelId}/events?access_token=${accessToken}`,
      {
        method: 'POST',
